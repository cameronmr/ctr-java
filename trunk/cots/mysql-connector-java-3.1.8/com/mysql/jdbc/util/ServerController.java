 ...
 *
 * @version $Id: ServerController.java,v 1.1.2.5 2004/09/17 01:11:51 mmatthew Exp $
...
public class ServerController {
	...
	/**
	 * What is the path to the mysql server executable (if
	 * not standard?)
	 */
	...
	/**
	 * Creates a server controller for the MySQL server with
	 * the given basedir and datadir.
	 * 
	...
	/**
	 * Starts the server, returning a java.lang.Process instance
	 * that represents the mysql server.
	 * 
	 * @return Process a java.lang.Process instance representing the
	 ...
	 * 
	 * @param forceIfNecessary use forceStop if mysqladmin doesn't shut 
	 * the server down
	 * 
	...
	/**
	 * Returns the list of properties that will be used
	 * to start/control the server.
	 * 
	...
	/**
	 * Returns the full commandline used to start the 
	 * mysql server, including and arguments to be passed
	...
	/**
	 * Builds the list of command-line arguments that will be passed
	 * to the mysql server to be started.
	 * 
	...
	private String buildOptionalCommandLine() {
		...
		if (this.serverProps != null) {
			...
			for (Iterator iter = this.serverProps.keySet().iterator(); iter.hasNext();) {
	...
	/**
	 * Returns true if the property does not belong as
	 * a command-line argument
	 * 
	 * @return boolean if the property should not be a 
	...
	/**
	 * Is this ServerController running on a Windows
	 * operating system?
	 * 
