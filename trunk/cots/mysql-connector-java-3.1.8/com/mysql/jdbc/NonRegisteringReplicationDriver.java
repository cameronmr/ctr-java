 ...
 * is read-only.
 * 
 * @version $Id: NonRegisteringReplicationDriver.java,v 1.1.2.1 2004/10/21 21:03:48 mmatthew Exp $
 */
public class NonRegisteringReplicationDriver extends NonRegisteringDriver {
	...
	public Connection connect(String url, Properties info) throws SQLException {
		...
		Properties masterProps = parsedProps;
		Properties slavesProps = parsedProps;
		...
		if (hostValues != null) {
			...
			if (slaveHosts.length() == 0) {
				throw new SQLException("Must specify at least one slave host to connect to for master/slave replication load-balancing functionality", 
