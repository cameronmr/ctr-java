 ...
 * handling exceptions.
 * 
 * @version $Id: MysqlConnectionTester.java,v 1.1.2.2 2005/03/22 00:45:07 mmatthews Exp $
 */
public final class MysqlConnectionTester implements QueryConnectionTester {
	...
	public MysqlConnectionTester() {
		try {
			pingMethod = com.mysql.jdbc.Connection.class.getMethod("ping", null);
		} catch (Exception ex) {
			// punt, we have no way to recover, other than we now use 'SELECT 1' for
	...
	public int activeCheckConnection(Connection con) {
		...
		try {
			if (pingMethod != null) {
				castCon.rawConnectionOperation(pingMethod, C3P0ProxyConnection.RAW_CONNECTION, NO_ARGS_ARRAY);
			} else {
