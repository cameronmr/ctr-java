 ...
 * 
 * @version $Id: CompressedInputStream.java,v 1.1.2.9 2004/09/18 03:31:18 mmatthew Exp $
 */
class CompressedInputStream extends InputStream {
    ...
    private void getNextPacketFromServer() throws IOException {
        ...
        if (this.connection.getTraceProtocol()) {
            try {
                this.connection.getLog().logTrace("Reading compressed packet of length "
                    + compressedPacketLength + " uncompressed to "
                    + uncompressedLength);
            } catch (SQLException sqlEx) {
                throw new IOException(sqlEx.toString()); // should never happen
            }
        }
