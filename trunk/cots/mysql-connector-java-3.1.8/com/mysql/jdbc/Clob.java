 ...
 * @version $Id: Clob.java,v 1.5.4.10 2004/08/12 20:28:29 mmatthew Exp $
 */
public class Clob implements java.sql.Clob, OutputStreamWatcher, WriterWatcher {
    ...
    public long position(String stringToFind, long startPos)
        throws SQLException {
        if (startPos < 1) {
            throw new SQLException(Messages.getString("Clob.8") //$NON-NLS-1$
    ...
    public void streamClosed(WatchableOutputStream out) {
        ...
        if (streamSize < this.charData.length()) {
            try {
                out.write(StringUtils.getBytes(this.charData, null, null, false),
    ...
    public void truncate(long length) throws SQLException {
        if (length > this.charData.length()) {
            throw new SQLException(Messages.getString("Clob.11") //$NON-NLS-1$
                + this.charData.length() + Messages.getString("Clob.12") + length + Messages.getString("Clob.13")); //$NON-NLS-1$ //$NON-NLS-2$
        }
    ...
    public void writerClosed(WatchableWriter out) {
        ...
        if (dataLength < this.charData.length()) {
            out.write(this.charData, dataLength,
                this.charData.length() - dataLength);
        }
