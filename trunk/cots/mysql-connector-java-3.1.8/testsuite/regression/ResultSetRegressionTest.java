...
public class ResultSetRegressionTest extends BaseTestCase {
    ...
    /**
     * Tests fix for BUG#7715 - Timestamps converted incorrectly to strings 
     * with SSPS and Upd. Result Sets.
     * 
     * @throws Exception if the test fails.
     */
    ...
    /**
     * Tests fix for BUG#8428 - getString() doesn't maintain format
     * stored on server.
     * 
     * @throws Exception if the test fails.
     */
    public void testBug8428() throws Exception {
    	...
    	try {
    		...
    		this.stmt.executeUpdate("CREATE TABLE testBug8428 (field1 YEAR, field2 DATETIME)");
    ...
    /**
     * Tests fix for Bug#8868, DATE_FORMAT() queries returned as BLOBs
     * from getObject().
     *  
     * @throws Exception if the test fails.
     */
    public void testBug8868() throws Exception {
    	try {
    		this.rs = this.stmt.executeQuery("SELECT DATE_FORMAT(NOW(),'%b-%e %l:%i%p') as fmtddate");
    		this.rs.next();
    ...
    /**
     * Tests fix for BUG#9236, a continuation of BUG#8868, where functions
     * used in queries that should return non-string types when resolved by temporary
     * tables suddenly become opaque binary strings (work-around for server limitation)
     * 
     * @throws Exception if the test fails.
     */
		...
		if (versionMeetsMinimum(4, 1)) {
	    	try {
	    		createTable("testBug9236", "(" 
	    			...
	    			this.rs = this.stmt.executeQuery("select DATE_FORMAT(field_12, '%Y-%m-%d') as date, count(*) as count from testBug9236 where field_10 = 0 and field_3 = 'FRL' and field_12 >= '2005-03-02 00:00:00' and field_12 <= '2005-03-17 00:00:00' group by date");
    ...
    public void testAllTypesForNull() throws Exception {
		...
		boolean needsNow = ((Boolean)wasDatetimeTypeList.get(0)).booleanValue();
    ...
    private void testAllFieldsForNotNull(ResultSet rsToTest, List wasDatetimeTypeList) throws Exception {
    	ResultSetMetaData rsmd = rs.getMetaData();
    	int numCols = rsmd.getColumnCount();
    	
		while (rsToTest.next()) {
			for (int i = 0; i < numCols - 1; i++) {
				boolean wasDatetimeType = ((Boolean)wasDatetimeTypeList.get(i)).booleanValue();
				if (!"BIT".equalsIgnoreCase(rsmd.getColumnTypeName(i + 1))) {
					if (!wasDatetimeType) {
					...
					boolean canBeUsedAsDate = !("java.lang.Boolean".equals(columnClassName)
							|| "java.lang.Double".equals(columnClassName) 
							|| "java.lang.Float".equals(columnClassName) 
							|| "java.lang.Real".equals(columnClassName)
    ...
    public void testNPEWithStatementsAndTime() throws Exception {
    	try {
    		...
    		this.stmt.executeUpdate("CREATE TABLE testNPETime (field1 TIME NULL, field2 DATETIME NULL, field3 DATE NULL)");
