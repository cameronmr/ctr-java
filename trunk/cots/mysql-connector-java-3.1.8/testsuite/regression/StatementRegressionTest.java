...
/*
	public void testBug9595() throws Exception {
		double[] vals = new double[] {52.21, 52.22, 52.23, 52.24};
		...
		// Try setting as doubles
		for (int i = 0; i < vals.length; i++) {
			this.pstmt.setDouble(1, vals[i]);
			this.pstmt.setInt(2, i);
			this.pstmt.executeUpdate();
		}
		
		this.pstmt = this.conn.prepareStatement("SELECT field1 FROM testBug9595 ORDER BY sortField");
		this.rs = this.pstmt.executeQuery();
		
		int i = 0;
		
		while (this.rs.next()) {
			double valToTest = vals[i++];
		...
		this.stmt.executeUpdate("TRUNCATE TABLE testBug9595");
		
		// Now, as BigDecimals
		for (i = 0; i < vals.length; i++) {
			BigDecimal foo = new BigDecimal(vals[i]);

			this.pstmt.setObject(1, foo, Types.DECIMAL, 2);
			this.pstmt.setInt(2, i);
			this.pstmt.executeUpdate();
		}
