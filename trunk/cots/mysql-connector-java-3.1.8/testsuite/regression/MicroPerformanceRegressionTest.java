 ...
 * 
 * @version $Id: MicroPerformanceRegressionTest.java,v 1.1.4.7 2004/11/05 16:30:00 mmatthew Exp $
 */
public class MicroPerformanceRegressionTest extends BaseTestCase {
    ...
    public void testResultSetAccessors() throws Exception {
        try {
            this.stmt.executeUpdate(
                "DROP TABLE IF EXISTS marktest");
            this.stmt.executeUpdate(
                "CREATE TABLE marktest(intField INT, floatField DOUBLE, timeField TIME, datetimeField DATETIME, stringField VARCHAR(64))");
            this.stmt.executeUpdate(
            ...
            this.rs = this.stmt.executeQuery(
                    "SELECT intField, floatField, timeField, datetimeField, stringField FROM marktest");
            ...
            if (versionMeetsMinimum(4, 1)) {
            	numPrepares = 100000; // we don't need to do so many for server-side prep statements...
            }
            ...
            for (int i = 0; i < numPrepares; i++) {
                ...
                PreparedStatement pStmt = this.conn.prepareStatement(
                        "INSERT INTO test.marktest VALUES (?, ?, ?, ?, ?)");
                pStmt.close();
            }
	...
	private void checkTime(String testType, double avgExecTimeMs) throws Exception {
		System.out.println("Execution time for " + testType + ": " + avgExecTimeMs);
	...
	public void testBug6359() throws Exception {
		if (runLongTests()) {
			...
			try {
				...
				this.stmt.executeUpdate("CREATE TABLE testBug6359 (pk_field INT PRIMARY KEY NOT NULL AUTO_INCREMENT, field1 INT, field2 INT, field3 INT, field4 INT, field5 INT, field6 INT, field7 INT, field8 INT, field9 INT,  INDEX (field1))");
				
				PreparedStatement pStmt = this.conn.prepareStatement("INSERT INTO testBug6359 (field1, field2, field3, field4, field5, field6, field7, field8, field9) VALUES (?, 1, 2, 3, 4, 5, 6, 7, 8)");
