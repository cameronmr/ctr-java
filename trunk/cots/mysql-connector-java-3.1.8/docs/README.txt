  ...
  Mark Matthews
   ...
   1. Developing Applications with MySQL and Java using Connector/J
                ...
                1.3. JDBC Reference
                             ...
                             1.3.1. Driver/Datasource Class Names, URL Syntax
   ...
   1.1. Connection Properties

   1.2. Conversion Table

   1.3. MySQL Types to Java Types for ResultSet.getObject()

   1.4. MySQL to Java Encoding Name Translations

   List of Examples

   1.1. Obtaining a Connection From the DriverManager

   1.2. Using java.sql.Statement to Execute a SELECT Query

   1.3. Stored Procedure Example

   1.4. Using Connection.prepareCall()

   1.5. Registering Output Parameters

   1.6. Setting CallableStatement Input Parameters

   1.7. Retrieving Results and Output Parameter Values

   1.8. Retrieving AUTO_INCREMENT Column Values using
   Statement.getGeneratedKeys()

   1.9. Retrieving AUTO_INCREMENT Column Values using 'SELECT
   LAST_INSERT_ID()'

   1.10. Retrieving AUTO_INCREMENT Column Values in Updatable ResultSets

   1.11. Setting the CLASSPATH Under UNIX

   1.12. Using a Connection Pool with a J2EE Application Server

   1.13. Example of transaction with retry logic
...
Chapter 1. Developing Applications with MySQL and Java using Connector/J
   ...
   1.3. JDBC Reference
                ...
                1.3.1. Driver/Datasource Class Names, URL Syntax and
...
1.1. Introduction to JDBC Development
   ...
   section "Installing Connector/J".
   ...
   Hibernate , Spring's JDBC templates or Ibatis SQL Maps to do the majority
   of repetitive work and heavier lifting that is sometimes required with
   JDBC.
   ...
   here:
     ...
     o JDBC Basics - A tutorial from Sun covering beginner topics in JDBC

     o JDBC Short Course - A more in-depth tutorial from Sun and JGuru
...
1.2. Installing Connector/J
  ...
  1.2.1. Required Software Versions
   ...
   the directly specifying it with the commandline switch -cp when starting
   your JVM
 ...
 $ setenv CLASSPATH /path/to/mysql-connector-java-[version]-bin.jar:$CLASSPATH
   ...
   variable. This document does contain configuration examples for some J2EE
   application servers in the section named "Using Connector/J with J2EE and
...
|---------------------------------+------------------------------------------------------------------------------+---------+------------------------------------+--------|
|useCompression                   |Use zlib compression when communicating with the server (true/false)? Defaults|No       |false                               |3.1.0   |
|                                 |to 'false'.                                                                   |         |                                    |        |
|------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
...
|                                 |'read-only'?                                                                  |         |                                    |        |
|---------------------------------+------------------------------------------------------------------------------+---------+------------------------------------+--------|
|maxReconnects                    |Maximum number of reconnects to attempt if autoReconnect is true, default is  |No       |3                                   |1.1     |
|                                 |'3'.                                                                          |         |                                    |        |
|---------------------------------+------------------------------------------------------------------------------+---------+------------------------------------+--------|
|initialTimeout                   |If autoReconnect is enabled, the initial time to wait between re-connect      |No       |2                                   |1.1     |
...
|alwaysSendSetIsolation           |will only communicate with the database when the requested transaction        |No       |true                                |3.1.7   |
|                                 |isolation is different than the whichever is newer, the last value that was   |         |                                    |        |
|                                 |set via Connection.setTransactionIsolation(), or the value that was read from |         |                                    |        |
|                                 |the server when the connection was established.                               |         |                                    |        |
|---------------------------------+------------------------------------------------------------------------------+---------+------------------------------------+--------|
|blobSendChunkSize                |Chunk to use when sending BLOB/CLOBs via ServerPreparedStatements             |No       |1048576                             |3.2.1   |
|---------------------------------+------------------------------------------------------------------------------+---------+------------------------------------+--------|
...
|                                 |(true/false) defaults to 'false'                                              |         |                                    |        |
|---------------------------------+------------------------------------------------------------------------------+---------+------------------------------------+--------|
|profileSql                       |Deprecated, use 'profileSQL' instead. Trace queries and their execution/fetch |No       |                                    |2.0.14  |
|                                 |times on STDERR (true/false) defaults to 'false'                              |         |                                    |        |
|---------------------------------+------------------------------------------------------------------------------+---------+------------------------------------+--------|
...
+------------------------------------------------------------------------------------------------------------------------------------------------------------------------+
 ...
 stmt.setFetchSize(Integer.MIN_VALUE);
     ...
     o Statement
       ...
       The "setFetchSize()" method has no effect, other than to toggle result
       set streaming as described above.
...
1.5. Diagnosing Connector/J Problems
  ...
  1.5.1. Common Problems and Solutions
   ...
   1.5.1.1. When I try to connect to the database with MySQL Connector/J, I
   get the following exception:

   1.5.1.2. My application throws a SQLException 'No Suitable Driver'. Why is
   this happening?

   1.5.1.3. I'm trying to use MySQL Connector/J in an applet or application
   and I get an exception similar to:

   1.5.1.4. I have a servlet/application that works fine for a day, and then
   stops working overnight

   1.5.1.5. I'm trying to use JDBC-2.0 updatable result sets, and I get an
