 ...
 * 
 * @version $Id: ExportControlled.java,v 1.1.2.7 2004/08/12 20:28:30 mmatthew Exp $
 */
public class ExportControlled {
    ...
    /**
     * Converts the socket being used in the given MysqlIO to 
     * an SSLSocket by performing the SSL/TLS handshake.
     * 
     * @param mysqlIO the MysqlIO instance containing the socket
     * to convert to an SSLSocket.
     * 
     * @throws CommunicationsException if the handshake fails, or
     * if this distribution of Connector/J doesn't contain the SSL
     * crytpo hooks needed to perform the handshake.
     */
    protected static void transformSocketToSSLSocket(MysqlIO mysqlIO)
        ...
        try {
            ...
            if (mysqlIO.connection.getUseUnbufferedInput()) {
            	 mysqlIO.mysqlInput = 
                        mysqlIO.mysqlConnection.getInputStream();
            } else {
