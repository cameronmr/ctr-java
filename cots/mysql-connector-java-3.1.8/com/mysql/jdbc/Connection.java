 ...
 * @version $Id: Connection.java,v 1.31.4.119 2005/04/07 18:40:45 mmatthews Exp $
 *
...
public class Connection extends ConnectionProperties
	...
	/**
	 * Used only when testing failover functionality for regressions,
	 * causes the failover code to not retry the master first
	 */
    ...
    public java.sql.PreparedStatement prepareStatement(String sql,
        ...
        if (this.useServerPreparedStmts && canServerPrepare) {
        	try {
        		pStmt = new com.mysql.jdbc.ServerPreparedStatement(this, sql,
                    this.database);
        	} catch (SQLException sqlEx) {
        		// Punt, if necessary
        		if (getEmulateUnsupportedPstmts()) {
        			pStmt = clientPrepareStatement(sql);
        		} else {
        			throw sqlEx;
        		}
        	}
    ...
    private void addToHistogram(int[] histogramCounts,
    	...
    	if (histogramCounts == null) {
            createInitialHistogram(histogramCounts, 
            		histogramBreakpoints,
            		currentLowerBound,
            		currentUpperBound);
        }
    ...
    private void addToPerformanceHistogram(long value, int numberOfTimes) {
    	...
    	addToHistogram(this.perfMetricsHistCounts, 
    			...
    			value,
    			numberOfTimes,
    			this.shortestQueryTimeMs == Long.MAX_VALUE ? 0 : this.shortestQueryTimeMs,
    		    this.longestQueryTimeMs);
    }
        ...
        if (serverEncoding != null) {
            ...
            if (mappedServerEncoding == null) {
            	throw new SQLException("Unknown character encoding on server '"
            			+ serverEncoding + "', use 'characterEncoding=' property "
            ...
            if (versionMeetsMinimum(4, 1, 0)) {
                ...
                if (getUseUnicode()) {
                	if (realJavaEncoding != null) {
	                    ...
	                    if (realJavaEncoding.equalsIgnoreCase("UTF-8")
	                        ...
	                        } else if ("euc_kr".equals(mysqlEncodingName)) {
	                        	// Different name in 4.1
	                        	mysqlEncodingName = "euckr";
	                        }
                ...
                if (getCharacterSetResults() == null) {
                	...
                	if ("UTF-8".equalsIgnoreCase(charsetResults)
                		...
                		mysqlEncodingName = (String) CharsetMapping.JAVA_UC_TO_MYSQL_CHARSET_MAP
                        	.get(charsetResults.toUpperCase(Locale.ENGLISH));
                	}
    ...
    private void createInitialHistogram(int[] counts, 
    		long[] breakpoints, 
    		long lowerBound, 
    		long upperBound) {
    	
        
        double bucketSize = (((double) upperBound
    	...
    	if (getGatherPerformanceMetrics()) {
    		...
    		if (this.perfMetricsHistBreakpoints != null) {
    			...
    			for (int i = 0; i < (HISTOGRAM_BUCKETS - 1); i++) {
    				...
    				if (i == 0) {
    					logMessage.append("\n\tless than "
    							+ this.perfMetricsHistBreakpoints[i + 1] + " ms: \t"
    		...
    		if (this.numTablesMetricsHistBreakpoints != null) {
    			...
    			for (int i = 0; i < (HISTOGRAM_BUCKETS - 1); i++) {
    				...
    				if (i == 0) {
    					logMessage.append("\n\t"
    							+ this.numTablesMetricsHistBreakpoints[i + 1] + " tables or less: \t\t"
    							+ this.numTablesMetricsHistCounts[i]);	
    				} else {
    					logMessage.append("\n\tbetween "
    						+ this.numTablesMetricsHistBreakpoints[i] + " and "
    ...
    private boolean shouldFallBack() {
        ...
        boolean tryFallback = ((secondsSinceFailedOver >= getSecondsBeforeRetryMaster())
        || (this.queriesIssuedFailedOver >= getQueriesBeforeRetryMaster()));
	...
	/**
	 * Is the server in a sql_mode that doesn't
	 * allow us to use \\ to escape things?
	 * 
	...
	private boolean canHandleAsServerPreparedStatement(String sql) {
		...
		if (StringUtils.startsWithIgnoreCaseAndNonAlphaNumeric(sql, "SELECT") ||
			...
			StringUtils.startsWithIgnoreCaseAndNonAlphaNumeric(sql, "UPDATE") ||
			StringUtils.startsWithIgnoreCaseAndNonAlphaNumeric(sql, "REPLACE")) {
			
			// check for limit ?[,?]
			
			/*
			 * The grammar for this (from the server) is:
			 * 
			 * ULONG_NUM
             * | ULONG_NUM ',' ULONG_NUM
			...
			while (currentPos < lastPosToLook) {
				int limitStart = StringUtils.indexOfIgnoreCaseRespectQuotes(
						currentPos, sql, "LIMIT ", quoteChar, allowBackslashEscapes);
