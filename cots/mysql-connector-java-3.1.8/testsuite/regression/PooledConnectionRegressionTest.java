 ...
 * the cache when the application finishes using it. When an error occurs,
...
public final class PooledConnectionRegressionTest extends BaseTestCase {
    ...
    /**
     * Tests fix for BUG#7136 ... Statement.getConnection() returning
     * physical connection instead of logical connection.
     */
    public void testBug7136() {
        ...
        try {
            ...
            Connection connFromStatement = conn.createStatement().getConnection();
            ...
            Connection connFromPreparedStatement = conn.prepareStatement("SELECT 1").getConnection();
    ...
    public void testPacketTooLargeException() throws Exception {
        ...
        try {
            ...
            this.stmt.executeUpdate(
                "CREATE TABLE testPacketTooLarge(field1 LONGBLOB)");

            Connection connFromPool = pc.getConnection();
            PreparedStatement pStmtFromPool = connFromPool.prepareStatement(
                    "INSERT INTO testPacketTooLarge VALUES (?)");
            ...
            byte[] bigBytes = new byte[(int) (maxAllowedPacket * 1.2)];

            try {
                pStmtFromPool.setBytes(1, bigBytes);
            ...
            // This should still work okay, even though the last query on the same
            // connection didn't...
