...
public class CallableStatementRegressionTest extends BaseTestCase {
	...
	/**
	 * Tests fix for BUG#9319 -- Stored procedures with same name in 
	...
	public void testBug9319() throws Exception {
		boolean doASelect = true; // SELECT currently causes the server to hang on the 
		...
		if (versionMeetsMinimum(5, 0, 2)) {
			if (isAdminConnectionConfigured()) {
				...
				try {
					...
					db1Connection.createStatement()
							.executeUpdate("DROP PROCEDURE IF EXISTS COMPROVAR_USUARI");
					db1Connection.createStatement()
							.executeUpdate("CREATE PROCEDURE COMPROVAR_USUARI(IN p_CodiUsuari VARCHAR(10),"
									+ "\nIN p_contrasenya VARCHAR(10),"
									+ "\nOUT p_userId INTEGER,"
									+ "\nOUT p_userName VARCHAR(30),"
									+ "\nOUT p_administrador VARCHAR(1))"
					...
					cstmt = db2Connection
					.prepareCall("{ call " + quoteChar + db1Connection.getCatalog() 
							+ quoteChar + "." + quoteChar + "COMPROVAR_USUARI" + quoteChar + "(?, ?, ?, ?, ?) }");
					cstmt.setString(1, "abc");
					cstmt.setString(2, "def");
					cstmt.registerOutParameter(3, java.sql.Types.INTEGER);
					cstmt.registerOutParameter(4, java.sql.Types.VARCHAR);
	...
	/*public void testBug9319() throws Exception {
		boolean doASelect = false; // SELECT currently causes the server to hang on the 
		                           // last execution of this testcase, filed as BUG#9405
		...
		if (versionMeetsMinimum(5, 0, 2)) {
			if (isAdminConnectionConfigured()) {
				...
				try {
					...
					db2Connection.createStatement().executeUpdate(
							"CREATE DATABASE IF NOT EXISTS db_9319");
					db2Connection.setCatalog("db_9319");
					...
					cstmt = db2Connection
					.prepareCall("{ call " + quoteChar + this.conn.getCatalog() 
							+ quoteChar + "." + quoteChar + "COMPROVAR_USUARI" + quoteChar + "(?, ?, ?, ?, ?) }");
					cstmt.setString(1, "abc");
					cstmt.setString(2, "def");
					cstmt.registerOutParameter(3, java.sql.Types.INTEGER);
					...
					if (db2Connection != null) {
						db2Connection.createStatement().executeUpdate(
								"DROP PROCEDURE IF EXISTS COMPROVAR_USUARI");
						// db2Connection.createStatement().executeUpdate(
						// "DROP DATABASE IF EXISTS db_9319");
					}
