 ...
 * @version $Id: MetaDataRegressionTest.java,v 1.3.4.33 2005/04/14 20:36:13 mmatthews Exp $
 */
public class MetaDataRegressionTest extends BaseTestCase {
    ...
    public void testBug2607() throws Exception {
        try {
            ...
            this.stmt.executeUpdate(
                "CREATE TABLE testBug2607 (field1 INT PRIMARY KEY)");
	...
	public void testBug2852() throws Exception {
	    try {
	        ...
	        assertTrue(rsmd.getColumnClassName(1).equals(this.rs.getObject(1)
	                                                       .getClass().getName()));
	        assertTrue("java.lang.Integer".equals(rsmd.getColumnClassName(1)));
	
	        assertTrue(rsmd.getColumnClassName(2).equals(this.rs.getObject(2)
	                                                       .getClass().getName()));
    ...
    public void testFixForBug1673() throws Exception {
        try {
            ...
            this.stmt.executeUpdate(
                "CREATE TABLE testBug1673 (field_1 INT, field_2 INT)");
            ...
            assertTrue("Ordinal position in full column list of '"
                ...
                (ordinalPosOfCol2Full != 0) && (ordinalPosOfCol2Scoped != 0)
    ...
    public void testGetColumnsUnsigned() throws Exception {
        try {
            ...
            this.stmt.executeUpdate(
                "CREATE TABLE testGetUnsignedCols (field1 BIGINT, field2 BIGINT UNSIGNED)");
    ...
    public void testIsCaseSensitive() throws Exception {
        ...
        if (versionMeetsMinimum(4, 1)) {
            try {
                this.stmt.executeUpdate(
                    "DROP TABLE IF EXISTS testIsCaseSensitiveCs");
                this.stmt.executeUpdate("CREATE TABLE testIsCaseSensitiveCs ("
                ...
                this.rs = this.stmt.executeQuery(
                        "SELECT bin_char, bin_varchar, ci_char, ci_varchar, bin_tinytext, bin_text, bin_med_text, bin_long_text, ci_tinytext, ci_text, ci_med_text, ci_long_text FROM testIsCaseSensitiveCs");
	...
	/** 
	 * Tests fix for BUG#6399, ResultSetMetaData.getDisplaySize() is wrong
	...
	public void testBug6399() throws Exception {
		if (versionMeetsMinimum(4, 1)) {
			try {
				...
				this.stmt.executeUpdate("CREATE TABLE testBug6399 (field1 CHAR(3) CHARACTER SET UTF8, field2 CHAR(3) CHARACTER SET LATIN1, field3 CHAR(3) CHARACTER SET SJIS)");
	...
	/**
	 * Tests fix for BUG#8792 - DBMD.supportsResultSetConcurrency() not returning
	...
	/**
	 * Tests fix for BUG#8803, 'DATA_TYPE' column from DBMD.getBestRowIdentifier()
	...
	public void testBug8803() throws Exception {
		...
		try {
			this.rs =  metadata.getBestRowIdentifier(this.conn.getCatalog(), null,
				tableName, DatabaseMetaData.bestRowNotPseudo, true);
	...
	/**
	 * Tests fix for BUG#9769 - Should accept null for procedureNamePattern, even
	 * though it isn't JDBC compliant, for legacy's sake.
	 * 
	 * @throws Exception if the test fails.
	 */
