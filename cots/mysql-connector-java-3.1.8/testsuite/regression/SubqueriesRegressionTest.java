 ...
 * @version $Id: SubqueriesRegressionTest.java,v 1.1.2.6 2004/09/18 03:31:17 mmatthew Exp $
 */
public class SubqueriesRegressionTest extends BaseTestCase {
    ...
    public void testSubQuery4() throws Exception {
        ...
        if (versionMeetsMinimum(4, 1)) {
            for (int i = 0; i < REPETITIONS; i++) {
                ...
                try {
                    this.rs = this.stmt.executeQuery(
                            "select colA, '' from t2 union select colA, colB from t3");
    ...
    private void createTables() throws Exception {
        ...
        this.stmt.executeUpdate(
            "create table t1(colA varchar(10), colB decimal(3,0))");
        this.stmt.executeUpdate(
            "create table t2(colA varchar(10), colB varchar(10))");
        this.stmt.executeUpdate(
            "create table t3(colA varchar(10), colB varchar(10))");
        this.stmt.executeUpdate(
            "create table t4(colA varchar(10), colB varchar(10))");
        this.stmt.executeUpdate(
            "insert into t1 values ('abcd', 1), ('efgh', 2), ('ijkl', 3)");
        this.stmt.executeUpdate(
            "insert into t2 values ('type1', '1'), ('type2', '2'), ('type3', '3')");
        this.stmt.executeUpdate(
            "insert into t3 values ('aaaa', 'abcd'), ('bbbb', 'efgh'), ('cccc', 'ijkl')");
        this.stmt.executeUpdate(
            "insert into t4 values ('abcd', 'type1'), ('efgh', 'type2'), ('ijkl', 'type3')");
    }
